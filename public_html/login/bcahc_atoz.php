...
if ( preg_match('|^http://gateway.tx.ovid.com/atoz/az/[A-Za-z0-9_-]*\?idtok=-1060706634|', $_SERVER['HTTP_REFERER']) ) {
  ...
  if ( preg_match("|^http://gateway.ovid.com/ovidweb.cgi|", $_GET['url']) ) {
    ...
    $jumpstart .= "&ID=bcahc999&PASSWORD=experts";
  ...
  elseif ( preg_match("|^http://search.ebscohost.com|", $_GET['url']) 
    || preg_match("|^http://search.epnet.com|", $_GET['url']) ) {
      ...
      header("Location: " . $arr[1] . "&user=bcahc&password=remote");
  }
