...
Drupal.gmap.addHandler('gmap', function (elem) {
  ...
  obj.bind('prepareshape', function (shape) {
    ...
    if (shape.type === 'circle' ) {
      if ( Object.prototype.toString.call(shape.center) === '[object Array]') { shape.center = new google.maps.LatLng(shape.center[0], shape.center[1]); } // otherwise it should be a LatLng already
      if ( shape.point2 ) {
        if ( Object.prototype.toString.call(shape.point2) === '[object Array]') { shape.point2 = new google.maps.LatLng(shape.point2[0], shape.point2[1]); } // otherwise it should be a LatLng already
    ...
    if (shape.type === 'rpolygon') { /* this is deprecated as we have circle now.  It is left for backwards compatibility */
      ...
      $.each(shape.points, function (i, n) {
        if ( Object.prototype.toString.call(n) === '[object Array]') { n = new google.maps.LatLng(n[0], n[1]); } // otherwise it should be a LatLng already
        pa.push( n );
      });
    }
    ...
    switch (shape.type) {
      case 'circle':
        ...
        if ( shape.outline ) { cargs.strokeColor = shape.color; } // outline color
        ...
        if ( shape.opacity ) { cargs.strokeOpacity = shape.color; cargs.fillOpacity = shape.color; } // shape opacity
      ...
      case 'polygon':
        ...
        if ( shape.outline ) { cargs.strokeColor = shape.strokeColor; } // Stroke color
        if ( shape.strokeWeight ) { cargs.strokeWeight = shape.strokeWeight; } // Stroke Weight
        if ( shape.strokeOpacity ) { cargs.strokeOpacity = shape.strokeOpacity; } // Stroke Opacity
        if ( shape.fillColor ) { cargs.fillColor = shape.fillColor; } // Fill Color
        if ( shape.fillOpacity ) { cargs.fillOpacity = shape.fillOpacity; } // Fill Opacity
      ...
      case 'encoded_line':
        ...
        if ( shape.opacity ) { cargs.strokeOpacity = shape.color; cargs.fillOpacity = shape.color; }
